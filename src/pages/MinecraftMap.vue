...
import L, { Map as LMap, ImageOverlay, LatLng, LayerGroup } from 'leaflet'
...
const useDemo = String(import.meta.env.VITE_POSITIONS_DEMO || '0') === '1'
...
function latLngFromXZ(x: number, z: number): LatLng {
  ...
  const yPx = imgH.value / 2 + (positions.isZPositiveDown ? z : -z)
  return L.latLng(yPx, xPx)
}

function buildMarkerHtml(username: string) {
  const imgSrc = '/images/steve-head.png'
  return (
    '<div class="mc-pin">' +
...
function initMap() {
  ...
  img.onload = async () => {
    ...
    // mapping + start data after image is ready (so demo knows bounds)
    await positions.loadUserMappingForStructure()
    if (useDemo) positions.startDemo({ width: imgW.value, height: imgH.value })
...
watch(() => positions.lastUpdateAt, rebuildMarkers)
watch(() => positions.onlineOnly, rebuildMarkers)
watch(() => positions.q, rebuildMarkers)
</script>
